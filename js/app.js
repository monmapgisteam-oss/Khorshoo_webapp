require([
  ...
  const GROUPS = [
    { id: 1, name: "Худаг, уст цэг, усан хангамж" },
    { id: 2, name: "Тэжээлийн ургамал, үйлдвэрлэл" },
    { id: 3, name: "Мал бордох аж ахуй" },
    { id: 4, name: "Мах, сүү, цагаан идээ" },
    { id: 5, name: "Үржлийн мал бойжуулах" },
    { id: 6, name: "Ноос, ноолуур, арьс шир" },
    { id: 7, name: "Дотор мах боловсруулах" },
    { id: 8, name: "Агуулах, зоорь, хөргүүр" },
    { id: 9, name: "Мал төхөөрөх цех" },
    { id: 10, name: "Мал үржүүлэг, технологи" },
    { id: 11, name: "Мал эмнэлгийн үйлчилгээ" },
    { id: 12, name: "Дундын техник үйлчилгээ" },
    { id: 13, name: "Гахай, шувуу, зөгий" },
    { id: 14, name: "ХАА түүхий эд, тээвэр" },
  ];
  const groupName = {};
  ...
  const uachShort = (v) => {
    const m = v && String(v).match(/(\d+)/);
    return m ? v + " — " + (groupName[+m[1]] || "") : v;
  };

  const SPECIFIC = {
    1: [
      ["F1_8_", "Мал услах зай ойртсон (км)"],
      ["F1_9_1_", 'Ус ойртсоноор "тарга хүчний өөрчлөлт" (%)'],
      ["F1_9_2_", 'Ус ойртсоноор "сүүний гарцын өөрчлөлт" (%)'],
      ["F1_10_1_", "Ундаалж буй өрхийн тоо"],
      ["F1_10_2_", "Ундаалж буй малын тоо (толгой)"],
      ["F1_11_", "Санал зөвлөмж"],
    ],
    2: [
      ["F2_8_", "Тэжээлийн ургамал тариалсан талбай (га)"],
      ["F2_9_", "Хэрэгцээг хангаж буй (%)"],
      ["F2_10_", "Бэлэн тэжээлийн зардлын бууралт (₮/жил)"],
      ["F2_11_", "Цехийн хүчин чадал (кг/өдөр)"],
      ["F2_12_", "Тэжээл/ургамал борлуулсан орлого (₮)"],
      ["F2_13_1_", "Трактор — тоо"],
      ["F2_13_1_1", "Трактор — мөнгөн дүн (₮)"],
      ["F2_13_2_", "Хаман боогч — тоо"],
      ["F2_13_2_1", "Хаман боогч — мөнгөн дүн (₮)"],
      ["F2_13_3_", "Тармуур — тоо"],
      ["F2_13_3_1", "Тармуур — мөнгөн дүн (₮)"],
      ["F2_13_4_", "Хадуур — тоо"],
      ["F2_13_4_1", "Хадуур — мөнгөн дүн (₮)"],
      ["F2_13_5_", "Бутлуур — тоо"],
      ["F2_13_5_1", "Бутлуур — мөнгөн дүн (₮)"],
      ["F2_13_6_", "Хашаажуулалт — тоо"],
      ["F2_13_6_1", "Хашаажуулалт — мөнгөн дүн (₮)"],
      ["F2_13_7_", "Бусад — тоо"],
      ["F2_13_7_1", "Бусад — мөнгөн дүн (₮)"],
      ["F2_13_7_2", "Бусад — тайлбар"],
      ["F2_14_", "Санал зөвлөмж"],
    ],
    3: [
      ["F3_8_1_", "Бордсон мал — Бод (толгой)"],
      ["F3_8_2_", "Бордсон мал — Бог (толгой)"],
      ["F3_9_", "Бүтээгдэхүүн нийлүүлэх гэрээ"],
      ["F3_10_", "Брэнд бүтээгдэхүүнтэй болсон эсэх"],
      ["F3_11_", "Санал зөвлөмж"],
    ],
    4: [
      ["F4_8_1_", "Сүү боловсруулах төхөөрөмж — тоо"],
      ["F4_8_1_1", "Сүү — мөнгөн дүн (₮)"],
      ["F4_8_2_", "Мах боловсруулах төхөөрөмж — тоо"],
      ["F4_8_2_1", "Мах — мөнгөн дүн (₮)"],
      ["F4_9_1_", "Хүчин чадал — сүү (литр/хоног)"],
      ["F4_9_2_", "Хүчин чадал — мах (кг/хоног)"],
      ["F4_10_", "Бүтээгдэхүүн нийлүүлэх гэрээ"],
      ["F4_11_", "Санал зөвлөмж"],
    ],
    5: [
      ["F5_9_", "Зээлээ зориулалтын дагуу зарцуулсан эсэх"],
      ["F5_10_1_", "Адуу — тоо"],
      ["F5_10_1_1", "Адуу — үнийн дүн (₮)"],
      ["F5_10_2_", "Үхэр — тоо"],
      ["F5_10_2_1", "Үхэр — үнийн дүн (₮)"],
      ["F5_10_3_", "Тэмээ — тоо"],
      ["F5_10_3_1", "Тэмээ — үнийн дүн (₮)"],
      ["F5_10_4_", "Хонь — тоо"],
      ["F5_10_4_1", "Хонь — үнийн дүн (₮)"],
      ["F5_10_5_", "Ямаа — тоо"],
      ["F5_10_5_1", "Ямаа — үнийн дүн (₮)"],
      ["F5_11_", "Үржлийн малаас олсон ашиг (₮)"],
      ["F5_12_", "Санал зөвлөмж"],
    ],
    6: [
      ["F6_8_", "Цех байгуулахад зарцуулсан зардал (₮)"],
      ["F6_9_1_", "Арьс ширний төхөөрөмж — тоо"],
      ["F6_9_1_1", "Арьс шир — мөнгөн дүн (₮)"],
      ["F6_9_2_", "Ноос ноолуурын төхөөрөмж — тоо"],
      ["F6_9_2_1", "Ноос ноолуур — мөнгөн дүн (₮)"],
      ["F6_10_", "Хүчин чадал (хоногт/хэмжээ)"],
      ["F6_11_", "Бэлэн бүтээгдэхүүн борлуулж буй эсэх"],
      ["F6_12_1_", "Борлуулсан — арьс шир (ширхэг)"],
      ["F6_12_2_", "Борлуулсан — ноос (кг)"],
      ["F6_12_3_", "Борлуулсан — ноолуур (кг)"],
      ["F6_13_", "Бүтээгдэхүүн нийлүүлэх гэрээ"],
      ["F6_14_", "Санал зөвлөмж"],
    ],
    7: [
      ["F7_8_", "Зээлээ зарцуулсан барилга байгууламж"],
      ["F7_8_1_", "Бусад (тайлбар)"],
      ["F7_9_1_", "Дайвар бүтээгдэхүүн боловсруулах төхөөрөмж — тоо"],
      [
        "F7_9_1_1",
        "Дайвар бүтээгдэхүүн боловсруулах төхөөрөмж — мөнгөн дүн (₮)",
      ],
      ["F7_10_", "Хүчин чадал (хоногт/хэмжээ)"],
      ["F7_11_", "Бүтээгдэхүүн нийлүүлэх гэрээ"],
      ["F7_12_", "Санал зөвлөмж"],
    ],
    8: [
      ["F8_8_", "Зээлээ зарцуулсан барилга байгууламж"],
      ["F8_8_1_", "Бусад (тайлбар)"],
      ["F8_9_", "Хүчин чадал (талбай м²)"],
      ["F8_10_", "Түрээсээс нэмэлт орлого (₮)"],
      ["F8_11_", "Санал зөвлөмж"],
    ],
    9: [
      ["F9_8_", "Бүтээгдэхүүн нийлүүлэх гэрээ"],
      ["F9_9_1_", "Хүчин чадал — бог (толгой/өдөр)"],
      ["F9_9_2_", "Хүчин чадал — бод (толгой/өдөр)"],
      ["F9_10_1_", "Нэмэлт ашиг — 1 бод (₮)"],
      ["F9_10_2_", "Нэмэлт ашиг — 1 бог (₮)"],
      ["F9_11_", "Дайвар бүт. борлуулж буй эсэх"],
      ["F9_12_", "Санал зөвлөмж"],
    ],
    10: [
      ["F10_9_1_", "Тоног төхөөрөмж — тоо ширхэг"],
      ["F10_9_2_", "Тоног төхөөрөмж — үнийн дүн (₮)"],
      ["F10_10_", "Санал зөвлөмж"],
    ],
    11: [
      ["F11_8_1_", "Тоног төхөөрөмж — тоо ширхэг"],
      ["F11_8_2_", "Тоног төхөөрөмж — үнийн дүн (₮)"],
      ["F11_9_1_", "Үзүүлсэн үйлчилгээний тоо"],
      ["F11_9_2_", "Үйлчилгээнээс орлого (₮)"],
      ["F11_10_", "Санал зөвлөмж"],
    ],
    12: [
      ["F12_8_1_", "Трактор — тоо"],
      ["F12_8_1_1", "Трактор — мөнгөн дүн (₮)"],
      ["F12_8_2_", "Хаман боогч — тоо"],
      ["F12_8_2_1", "Хаман боогч — мөнгөн дүн (₮)"],
      ["F12_8_3_", "Тармуур — тоо"],
      ["F12_8_3_1", "Тармуур — мөнгөн дүн (₮)"],
      ["F12_8_4_", "Хадуур — тоо"],
      ["F12_8_5_", "Бутлуур — тоо"],
      ["F12_8_5_1", "Бутлуур — мөнгөн дүн (₮)"],
      ["F12_8_6_", "Хөргүүртэй тээвэр — тоо"],
      ["F12_8_6_1", "— мөнгөн дүн (₮)"],
      ["F12_8_7_", "Ачаа тээврийн техник — тоо"],
      ["F12_8_7_1", "— мөнгөн дүн (₮)"],
      ["F12_8_8_", "Ковш тусгай техник — тоо"],
      ["F12_8_8_1", "— мөнгөн дүн (₮)"],
      ["F12_8_9_", "Бусад — тоо"],
      ["F12_8_9_1", "Бусад — мөнгөн дүн (₮)"],
      ["F12_9_", "Техникээ түрээслүүлж олсон орлого (₮/жил)"],
      ["F12_10_", "Санал зөвлөмж"],
    ],
    13: [
      ["F13_8_1_", "Гахай — тоо"],
      ["F13_8_1_1", "Гахай — мөнгөн дүн (₮)"],
      ["F13_8_2_", "Шувуу — тоо"],
      ["F13_8_2_1", "Шувуу — мөнгөн дүн (₮)"],
      ["F13_8_3_", "Зөгий — бүл"],
      ["F13_8_3_1", "Зөгий — мөнгөн дүн (₮)"],
      ["F13_8_4_", "Цаа буга — тоо"],
      ["F13_8_4_1", "Цаа буга — мөнгөн дүн (₮)"],
      ["F13_8_5_", "Бусад — тоо"],
      ["F13_8_5_1", "Бусад — мөнгөн дүн (₮)"],
      ["F13_9_", "Санал зөвлөмж"],
    ],
    14: [
      ["F14_8_1_", "Трактор — тоо"],
      ["F14_8_1_1", "Трактор — мөнгөн дүн (₮)"],
      ["F14_8_2_", "Хөргүүртэй тээвэр — тоо"],
      ["F14_8_2_1", "— мөнгөн дүн (₮)"],
      ["F14_8_3_", "Ачаа тээврийн техник — тоо"],
      ["F14_8_3_1", "— мөнгөн дүн (₮)"],
      ["F14_8_4_", "Ковш тусгай техник — тоо"],
      ["F14_8_4_1", "— мөнгөн дүн (₮)"],
      ["F14_8_5_", "Бусад — тоо"],
      ["F14_8_5_1", "Бусад — мөнгөн дүн (₮)"],
      ["F14_8_5_2", "Бусад — тайлбар"],
      ["F14_9_", "Техникээ түрээслүүлж олсон орлого (₮/жил)"],
      ["F14_10_", "Зээлээ зарцуулсан барилга"],
      ["F14_10_1_", "Бусад (тайлбар)"],
      ["F14_11_", "Санал зөвлөмж"],
    ],
  };
  ...
  function fmtNum(v) {
    ...
    if (!isNaN(n) && String(v).trim() !== "" && /^[\d., ]+$/.test(String(v)))
  ...
  function row(k, v) {
    return `<div class="pp-row"><div class="k">${k}</div><div class="v">${v}</div></div>`;
  }
  ...
  const NUMHINT =
    /км|%|тоо|толгой|₮|дүн|орлого|зардал|ашиг|литр|кг|\bга\b|өрх|ширхэг|бүл|м²|хувь/i;
  ...
  function indicatorHtml(big, unit, sub) {
    return `<div class="ind"><span class="big">${big}</span>${unit ? `<span class="unit">${unit}</span>` : ""}</div>${sub ? `<div class="ind-sub">${sub}</div>` : ""}`;
  }
  function hexA(hex, a) {
    const h = hex.replace("#", "");
  ...
  function donutSvg(segs, total) {
    ...
    segs.forEach((s) => {
      ...
      const o0 = pt(rO, a0),
        ...
        i1 = pt(rI, a1),
        i0 = pt(rI, a0);
      paths += `<path d="M${o0[0]} ${o0[1]} A${rO} ${rO} 0 ${large} 1 ${o1[0]} ${o1[1]} L${i1[0]} ${i1[1]} A${rI} ${rI} 0 ${large} 0 ${i0[0]} ${i0[1]} Z" fill="${hexA(s.color, 0.3)}" stroke="${s.color}" stroke-width="0.8"/>`;
      a0 = a1;
    });
  ...
  function donutHtml(segs, total) {
    return `<div class="donut-wrap">${donutSvg(segs, total)}<div class="dleg">${segs
      .map(
        (s) =>
          `<div class="r"><span class="sw" style="background:${s.color}"></span><span class="nm" title="${s.name}">${s.name}</span><span class="vv">${fmtCount(s.value)}</span></div>`,
      )
      .join("")}</div></div>`;
  }
  ...
  function histogramHtml(vals, isMoney) {
    ...
    vals.forEach((v) => {
      ...
      let idx = Math.floor((v.n - min) / step);
    ...
    for (let i = 0; i < BINS; i++)
      cells.push({
        lbl: fmtRange(min + step * i, min + step * (i + 1), isMoney),
        w: bins[i],
      });
    ...
    return cells
      .map((c) => {
        ...
        return (
          `<div class="hbar-row"><div class="hbar-label" title="${c.lbl}">${c.lbl}</div>` +
  ...
  function popupContent(feature) {
    ...
    const place = [
      a["Аймаг"] || a["Аймаг_Хот"],
      a["Зөв_сумын_нэр"] || a["Сум_Дүүрэг"] || a["Сум"],
    ]
      .filter(Boolean)
      .join(", ");
    ...
    if (!isEmpty(a["Зээлийн_зориулалт"]))
      html += row("Зээлийн зориулалт", a["Зээлийн_зориулалт"]);
    const person =
      [a["Овог"], a["Нэр"]].filter(Boolean).join(" ") ||
      a["Зээл_хүсэгчийн_овог_нэр"];
    if (!isEmpty(person)) html += row("Зээлдэгч", person);
    if (!isEmpty(a["Хүйс"]))
      html += row(
        "Хүйс / Нас",
        [a["Хүйс"], a["Нас"]].filter((v) => !isEmpty(v)).join(" / "),
      );
    html += `<div class="pp-section-title">Зээлийн мэдээлэл</div>`;
    if (!isEmpty(a["Олгосон_зээлийн_дүн"]))
      html += row("Олгосон зээл", fmtNum(a["Олгосон_зээлийн_дүн"]) + " ₮");
    if (!isEmpty(a["Зээлийн_үлдэгдэл"]))
      html += row("Зээлийн үлдэгдэл", fmtNum(a["Зээлийн_үлдэгдэл"]) + " ₮");
    if (!isEmpty(a["Банкны_нэр"])) html += row("Банк", a["Банкны_нэр"]);
    if (!isEmpty(a["Олгосон_огноо"]))
      html += row("Олгосон огноо", fmtDate(a["Олгосон_огноо"]));
    html += `<div class="pp-section-title">Асуумжийн хариулт</div>`;
    const fields = gid && SPECIFIC[gid] ? SPECIFIC[gid] : [];
    let any = false;
    fields.forEach(([f, label]) => {
      if (!isEmpty(a[f])) {
        any = true;
        const isMoney = /₮|дүн|орлого|зардал|ашиг/.test(label);
        html += row(label, isMoney ? fmtNum(a[f]) : a[f]);
      }
    });
    if (!any)
      html += `<div class="pp-empty">Энэ цэгт асуумжийн хариулт бөглөгдөөгүй байна.</div>`;
  ...
  const updateScale = () => {
    if (view.scale)
      scaleNode.textContent =
        "1:" + Math.round(view.scale).toLocaleString("en-US");
  };
  ...
  function whereParts(skipKey) {
    ...
    if (searchText) {
      ...
      parts.push(
        "(Хоршооны_нэр LIKE '%" +
          s +
          "%' OR Зээл_хүсэгчийн_овог_нэр LIKE '%" +
          s +
          "%' OR Нэр LIKE '%" +
          s +
          "%' OR Овог LIKE '%" +
  ...
  async function distinctWithCounts(field, skipKey) {
    const res = await surveyLayer.queryFeatures({
      ...
      orderByFields: field === "ҮАЧ" ? [field + " ASC"] : ["c DESC"],
      num: 1000,
    });
    let opts = res.features
      .map((f) => ({ value: f.attributes[field], count: f.attributes.c }))
  ...
  async function statTotals() {
    const res = await surveyLayer.queryFeatures({
      ...
      outStatistics: [
        ...
        {
          statisticType: "sum",
          onStatisticField: "Зээлийн_үлдэгдэл",
          outStatisticFieldName: "bal",
        },
      ],
    });
  ...
  function renderBars(elId, data, opts) {
    ...
    el.innerHTML = "";
    if (!data.length) {
      el.innerHTML = `<div class="empty">Мэдээлэл алга</div>`;
      return;
    }
    const max =
      Math.max.apply(
    ...
    data.forEach((d) => {
      ...
      r.className =
        "bar-row" + (opts.isActive && opts.isActive(d.key) ? " active" : "");
  ...
  function updateSummary(key) {
    ...
    if (s.size === 0) {
      sumEl.textContent = "Энд дарж ...";
      elBox.classList.remove("has-val");
      badge.textContent = "";
    } else if (s.size === 1) {
      sumEl.textContent = [...s][0];
      elBox.classList.add("has-val");
      badge.textContent = "1";
    } else {
      sumEl.textContent = s.size + " сонгосон";
      elBox.classList.add("has-val");
  ...
  async function refreshAll() {
    ...
    if (regionLayer)
      regionLayer.definitionExpression = inClause("name", sel.bus) || "";
    ...
    (async () => {
      try {
        if (where === "1=1") {
          view
            .goTo({ center: [103.5, 46.8], scale: 12000000 }, { duration: 600 })
            .catch(() => {});
          return;
        }
        ...
        if (targetScale) {
          view
            .goTo(
              { target: r.extent.center, scale: targetScale },
              { duration: 700 },
            )
            .catch(() => {});
        } else {
  ...
  async function updateIndicators(seq) {
    ...
    try {
      const [cLoan, totals, cSurvey] = await Promise.all([
        surveyLayer.queryFeatureCount({
          where: aw("Олгосон_зээлийн_дүн <> 0"),
        }),
      ...
      document.getElementById("ind-loan").textContent = fmtB(
        totals.features[0].attributes.loan || 0,
      );
  ...
  function niceCeil(x) {
    ...
    for (const k of [1, 2, 2.5, 5, 10]) {
      if (k * mag >= x) return k * mag;
    }
    return 10 * mag;
  }
  function fmtB(v) {
    const b = v / 1e9;
    if (Math.abs(b) >= 1) return b.toFixed(1).replace(/\.0$/, "") + "тэрбум";
    const m = v / 1e6;
    if (Math.abs(m) >= 1) return Math.round(m) + "сая";
  ...
  function fmtCompact(x) {
    ...
    if (a >= 1e9)
      return (
        (Math.round(x / 1e8) / 10).toString().replace(/\.0$/, "") + "тэрбум"
      );
    if (a >= 1e6)
      return (Math.round(x / 1e5) / 10).toString().replace(/\.0$/, "") + "сая";
    if (a >= 1e4)
      return (
        (Math.round(x / 100) / 10).toString().replace(/\.0$/, "") + "мянга"
      );
  ...
  const CHARTS = [
    ...
    {
      id: "loan-purpose",
      title: "Зээл олгосон дүн, зориулалтаар",
      render: (s) =>
        renderSerial(s, {
          field: "Зээлийн_зориулалт",
          extraWhere: "Зээлийн_зориулалт <> '-'",
          labelWidth: "46%",
          title: "Зээл олгосон дүн, зориулалтаар",
        }),
    },
  ...
  function renderLeftPanel() {
    ...
    const prev = document.getElementById("cs-prev"),
      next = document.getElementById("cs-next");
  ...
  async function renderSerial(s, opt) {
    ...
    const lwStyle =
      typeof lw === "number"
        ? `width:${lw}px;min-width:${lw}px`
        : `flex:0 0 ${lw};width:${lw};min-width:0`;
    let rows;
    try {
      let where = whereParts(opt.skipDim);
      if (opt.extraWhere)
        where =
          where === "1=1" ? opt.extraWhere : where + " AND " + opt.extraWhere;
      const res = await surveyLayer.queryFeatures({
        where: where,
        returnGeometry: false,
      ...
      rows = res.features
        .map((f) => ({
          key: f.attributes[opt.field],
          value: f.attributes.s || 0,
        }))
        .filter((r) => !isEmpty(r.key) && r.value > 0);
    } catch (e) {
      rows = [];
    }
    if (s !== leftSeq) return;
    ...
    if (!rows.length) {
      pl.innerHTML = html + `<div class="serial-empty">Мэдээлэл алга</div>`;
      return;
    }
    ...
    rows.forEach((r) => {
      ...
      const active =
        opt.clickDim && sel[opt.clickDim].has(r.key) ? " active" : "";
      const barCursor = opt.clickDim ? "" : "cursor:default;";
      html +=
        `<div class="s-row${active}" data-key="${escAttr(r.key)}">` +
    ...
    html += `</div>`;
    html +=
      `<div class="s-axis"><div class="sp" style="${lwStyle}"></div><div class="ax">` +
    ...
    if (opt.clickDim) {
      pl.querySelectorAll(".s-row").forEach((rowEl) => {
        rowEl.querySelector(".s-bar").onclick = () =>
          toggleSel(opt.clickDim, rowEl.dataset.key);
      });
    }
  }
  ...
  async function renderRightPanel(seq) {
    ...
    fields.forEach(([f, label], i) => {
      html += `<div class="rp-card" id="rpc-${i}"><div class="rp-q">${prettyQ(label)}</div><div class="rp-body"><span class="rp-empty">…</span></div></div>`;
    });
    pr.innerHTML = html;
    ...
    fields.forEach(async ([f, label], i) => {
      try {
        ...
        if (seq !== refreshSeq) return;
        const rows = res.features.map((ff) => ({
          key: ff.attributes[f],
          value: ff.attributes.cnt,
        }));
        ...
        if (!responded) {
          bodyEl.innerHTML = `<div class="rp-empty">Хариулт алга</div>`;
          return;
        }
        ...
        if (FREETEXT.test(label)) {
          if (/санал зөвлөмж/i.test(label)) {
            ...
            bodyEl.querySelector("button").onclick = (ev) => {
              ev.stopPropagation();
              openSuggestions(f, label, groupWhere);
            };
          } else {
            bodyEl.innerHTML = indicatorHtml(
              fmtCount(responded),
              "хариулт",
              "бичгээр ирүүлсэн",
            );
        ...
        const numFrac =
          clean
            .filter((r) => !isNaN(parseNum(cleanLabel(r.key))))
            .reduce((a, r) => a + r.value, 0) / responded;
        const numeric =
          isNumericField(f) ||
          NUMHINT.test(label) ||
          (numFrac >= 0.8 && clean.length >= 5);
        if (numeric) {
          const vals = [];
          clean.forEach((r) => {
            const n = parseNum(cleanLabel(r.key));
            if (!isNaN(n)) vals.push({ n: n, w: r.value });
          });
          ...
          const hist =
            new Set(vals.map((v) => v.n)).size >= 2
              ? histogramHtml(vals, isMoney)
              : null;
          bodyEl.innerHTML = hist
            ? hist + subline
            : indicatorHtml(
                fmt(sum),
                "нийт",
                `${fmtCount(responded)} хариулсан`,
              );
        } else {
          const agg = {};
          clean.forEach((r) => {
            const k = cleanLabel(r.key);
            agg[k] = (agg[k] || 0) + r.value;
          });
          let segs = Object.keys(agg)
  ...
  async function openSuggestions(field, label, where) {
    ...
    try {
      const res = await surveyLayer.queryFeatures({
        ...
        outFields: [field],
        returnGeometry: false,
        orderByFields: [OID + " DESC"],
        num: 1000,
      });
      if (seq !== sgSeq) return;
      const items = res.features
        .map((ft) => ft.attributes[field])
        .filter((v) => !isEmpty(v));
      ...
      listEl.innerHTML = items.length
        ? items
            .map(
              (t) =>
                `<div class="sg-item">${String(t).replace(/</g, "&lt;")}</div>`,
            )
            .join("")
        : `<div class="sg-empty">Санал алга</div>`;
    } catch (e) {
      if (seq === sgSeq)
        listEl.innerHTML = `<div class="sg-empty">Алдаа гарлаа</div>`;
    }
  }
  function closeSg() {
    document.getElementById("sg-panel").classList.remove("show");
    sgOpen = false;
  }
  document
    .getElementById("sg-panel")
  ...
  (function () {
    ...
    window.addEventListener("mousemove", (e) => {
      if (!mode) return;
      if (mode === "l" || mode === "tl") {
        let w = sw + (sx - e.clientX);
        w = Math.max(280, Math.min(window.innerWidth - 30, w));
        panel.style.width = w + "px";
      }
      if (mode === "t" || mode === "tl") {
        let h = sh + (sy - e.clientY);
        h = Math.max(160, Math.min(window.innerHeight - 90, h));
  ...
  function toggleSel(key, value) {
    if (sel[key].has(value)) sel[key].delete(value);
    else sel[key].add(value);
    updateSummary(key);
    refreshAll();
  }
  ...
  function buildSelectors() {
    ...
    Object.keys(DIMS).forEach((key) => {
      ...
      d.innerHTML =
        ICON +
        `<div class="sel-txt"><div class="sel-label">${DIMS[key].label}</div><div class="sel-sum">Энд дарж ...</div></div><span class="badge"></span>`;
      d.onclick = (e) => {
        e.stopPropagation();
        openDropdown(key, d);
      };
  ...
  function renderDdList() {
    ...
    const items = ddOptions.filter(
      (o) => !q || String(o.value).toLowerCase().indexOf(q) !== -1,
    );
    if (!items.length) {
      ddList.innerHTML = `<div class="dd-empty">Илэрц алга</div>`;
      return;
    }
    ddList.innerHTML = "";
    items.forEach((o) => {
      ...
      it.querySelector("input").onchange = (ev) => {
        ev.stopPropagation();
        if (ddKey === "uach") {
        ...
        if (ev.target.checked) sel[ddKey].add(o.value);
        else sel[ddKey].delete(o.value);
        updateSummary(ddKey);
        refreshAll();
      };
  ...
  async function openDropdown(key, anchor) {
    if (ddKey === key && dd.classList.contains("show")) {
      closeDropdown();
      return;
    }
    closeDropdown();
    ddKey = key;
    document.getElementById("dd-all").style.display =
      key === "uach" ? "none" : "";
    ...
    try {
      ddOptions = await distinctWithCounts(DIMS[key].field, key);
    } catch (e) {
      ddOptions = [];
    }
  ...
  function closeDropdown() {
    ...
    document
      .querySelectorAll(".sel.open")
      .forEach((s) => s.classList.remove("open"));
    ddKey = null;
  }
  ddSearch.addEventListener("input", renderDdList);
  document.getElementById("dd-reset").onclick = (e) => {
    e.stopPropagation();
    if (!ddKey) return;
    sel[ddKey].clear();
    updateSummary(ddKey);
  ...
  document.getElementById("dd-all").onclick = (e) => {
    e.stopPropagation();
    if (!ddKey) return;
    const q = ddSearch.value.trim().toLowerCase();
    ddOptions
      .filter((o) => !q || String(o.value).toLowerCase().indexOf(q) !== -1)
      .forEach((o) => sel[ddKey].add(o.value));
    updateSummary(ddKey);
    renderDdList();
    refreshAll();
  };
  ...
  (function () {
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      let w = Math.max(
        200,
        Math.min(window.innerWidth - 280, window.innerWidth - e.clientX),
      );
  ...
  document.getElementById("cs-prev").onclick = () => {
    chartIdx = (chartIdx - 1 + CHARTS.length) % CHARTS.length;
    renderLeftPanel().catch(() => {});
  };
  document.getElementById("cs-next").onclick = () => {
    chartIdx = (chartIdx + 1) % CHARTS.length;
    renderLeftPanel().catch(() => {});
  };
  ...
  view.when(async () => {
    ...
    if (!surveyLayer)
      surveyLayer =
        webmap.layers.find((l) => /khorshoo|asuumj/i.test(l.title || "")) ||
    ...
    (surveyLayer.fields || []).forEach((fl) => {
      fieldType[fl.name] = fl.type;
    });
    surveyLayer.outFields = ["*"];
    surveyLayer.popupTemplate = {
      title: "{Хоршооны_нэр}",
      outFields: ["*"],
